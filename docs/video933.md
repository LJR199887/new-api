...
fa2api `04a3054` 已在代码中接通图片、首尾帧、视频和音频参考，并完成离线回归。部署端必须升级到该版本或更新版本；若仍返回 `image-reference video currently requires...` 或媒体检查协议 `501`，说明线上 fa2api 还是旧版。含音视频的新链路尚未重新进行线上付费生成验证，上线前仍应使用实际账号各做一次四类参考请求的端到端验证。
