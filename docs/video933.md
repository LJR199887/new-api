...
| 模型 | 固定分辨率 | 生成时长 |
| --- | --- | --- |
| `933-video2.0` | 720p | 4、5 秒 |
| `933-video2.0-480p` | 480p | 4、5 秒 |
| `933-video2.0-mini` | 720p | 4、5 秒 |
| `933-video2.0-mini-480p` | 480p | 4、5 秒 |

时长默认 5 秒。比例：16:9、9:16、4:3、3:4、1:1、21:9，默认 16:9。显式分辨率与模型不一致会报错，不静默降档。
...
- 视频最多 3 个，每个 2–15 秒，视频总时长不大于 15 秒。
- 音频最多 3 个，每个 2–15 秒，音频总时长不大于 15 秒；与视频总时长分别统计。
...
- 后端复用纯 Go 媒体解析器，支持 MP4/MOV/WebM、MP3/WAV/M4A/FLAC/OGG 等已有格式；不能探测时长的文件拒绝提交，不信任客户端填写的时长。每个媒体下载限 200 MiB、30 秒，顺序使用临时文件，结束即删除；沿用现有 SSRF、重定向和 Worker 设置。
- 创作中心提供首尾帧、多图、视频、音频和混合参考入口。音视频 URL 会先在浏览器探测时长；文件上传依赖支持该类型的外部图床。没有外部图床时可使用素材 URL。常规图片上传仍保留原 10 MiB 上限，只有选择这四个模型时允许 20 MiB。
...
- 前端 4 项测试 / 33 个断言通过，包括新模型参数与修改模块的 JSX 解析。
